...
19. `node assert-pay-rows.mjs` — **REAL PAY**（[actual-pay.html](actual-pay.html)）の検査。
   ...
   **後ろ2つはまだページが無い。`<span aria-disabled="true">` で置き、`href` を書かない**
   ...
   ⑥並びはサーバーが決める（**出した順・古いほうが上**）⑦常識の幅（年 $10,000〜$700,000 の外は
   打ち間違いとして出さない）。
   ...
   前は `md5(人のキー)` 順＝並びに時間がまったく無い形だった。今は**出した順**（`order by last_at`）。
   段（5段）を出した時点で「だいたいいつ」は既に読めるが、**同じ段の中でもどちらが先かが読める**ようになった。
   これは正直に言って**段だけのときより一段弱い**。並べるのに使う時刻は行に入れていないし、
   画面から並べ替える口も無い。戻すのは `db/pay-rows.sql` の `order by p.last_at, md5(p.pkey)` を
   `order by md5(p.pkey)` に書き戻すだけ（新しい順にするなら `last_at desc`）。
   **同じ会社・同じ職位の同僚には当てられうる**水準であることは承知のうえで選んだ形。見るのは:
   ...
   - **並び替えの口を作らない**（`sort` / `新しい順` / `order` の select が日英の HTML と JS に無い）。
     並びを決めるのはサーバーだけ（出した順）。画面に口を作ると、金額順のランキングにされる
