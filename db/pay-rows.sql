...
--
-- ★鍵の無い人の一覧について（2026-09-16 オーナー指示）
--   給与をまだ1件も出していない人（未ログイン・会員登録だけ・口コミだけ）にも
--   **本物の一覧**を返す。ただし行に入れるのは 航空会社・出典・投稿時期 の3つだけで、
--   金額・職位・機材は**そもそも渡さない**（ぼかすのではない）。画面は渡らなかった欄に空の板を描く。
--   ★2026-09-21 オーナー指示「職位ださなくていいや」で職位も渡すのをやめた（4つ → 3つ）。
--   ⚠️ 2026-09-19〜09-21 のあいだだけ「上の8行を1人に固定の型（年収型／機種型／会社型）で出す」
--      形にしていた。給与の新規提出が2日続けて0になり、オーナー指示で 80ff719 を revert して
--      この形へ戻した。型の話を書き戻すなら、下の mask と actual-pay.js の maskedList と
--      assert-pay-rows.mjs の3か所が対（1つだけ直すと画面は動いたまま中身が食い違う）。
--
...
--   ・伏せた一覧 … far は返さない（画面に ⚠ も出ない）。伏せた一覧はそもそも金額を持たないので、
--                  印だけが残ると「額は見えないのに外れていることは分かる」形になる。
...
--   鍵の無い人の行は、この中から airline / verified / age の3つだけ（2026-09-21
--   オーナー指示で pos も外した）。
--   far は入れない（金額を渡していないので、印だけが残ると意味が変わる）。
--
...
begin
  ...
  mask as (
    ...
    /* ── 鍵の無い人へ返す行（2026-09-16 オーナー指示）───────────────
       ★**person からしか引かない。** 上の listed のように pick / paid /
         ...
         渡すのは3つだけ ── 会社・出典・投稿時期。
         ★2026-09-21 オーナー指示で**職位も渡すのをやめた**。画面は職位の欄に
           中身の空の板を置き、絞り込みの職位の口ごと隠す。送っていないから
           隠せる ── 送ったうえで描かない、にしない。
       ★金額と機材は「伏せて渡す」のではなく**渡さない**。画面はそこに
         中身の空の板を描くので、DevTools でぼかしを外しても何も出てこない。
       ★件数も並びも本物と同じ（同じ person・同じ順）。行を抜き差ししない
         ── 数え上げのカードと表の行数が食い違う。
       ⚠️ 2026-09-19〜09-21 のあいだだけ、ここで「上の8行を1人に固定の型（年収型／
          機種型／会社型）で出す」形にしていた。給与の新規提出が2日続けて0になり、
          2026-09-21 にオーナー指示で 80ff719 を revert してこの形へ戻した。
          書き戻すなら actual-pay.js の maskedList と assert-pay-rows.mjs も対で直す。
  ...
  select jsonb_build_object(
           ...
           -- ★行はここだけで落とす。鍵が無ければ mask の行（会社・出典・
           --   投稿時期の3つだけ）に差し替わる。金額・職位・機材は**渡さない**ので、
...
--   渡せるようにしたのは会社・出典・投稿時期の3つだけで、金額・職位・機材は
...
comment on function public.pv_pay_rows() is
  ...
  '会社・出典・投稿時期の3つだけで、年収・職位・機材・勤務・内訳は1つも入らない。'
  '2026-09-19 から、公開年収（同じ会社・同じ職位）の上限の1.5倍超か下限の半分未満の'
  '本人申告の人に far=true を付ける（数字は書き換えない）。伏せた行には far を入れない。';
...
from (
  ...
  --   下の65（渡すのは3つだけ）と66（渡っている相手は2つだけ）が持つ。
  ...
  --   mask が組んだ3つだけの行で、金額も職位も機材も入っていない（中身の検査は65）。
  ...
  select 65, '★伏せた行に渡すのは3つだけ（年収・職位・機材・勤務・内訳は組み立てもしない）',
         ...
         case when f_rows is null then false
                    ...
                    select s.x is not null
                       and s.x like '%''airline''%'
                       and s.x like '%''verified''%' and s.x like '%''age''%'
                       -- 職位は渡さない（2026-09-21 オーナー指示）
                       and s.x not like '%''pos''%'
  ...
  union all
  select 68, '★印（⚠）を返すのは本物の一覧だけ（伏せた一覧は fence を読まない）',
         /* 静かに壊れる。伏せた一覧が印だけを返すようになると、「額は見えないのに
            外れていることは分かる」行が鍵の無い人の最初の画面に出る。画面は何も変わらない。
            ⚠️ 2026-09-19〜09-21 は、ここで「mask が fence を読んでいること」
               （上の8行から外すため）も見ていた。2026-09-21 に元の形へ戻した
               （80ff719 の revert）ので、mask は fence をもう読まない。
               伏せた行に far が入らないことは自己点検65 が見ている。 */
         case when f_rows is null then false
              else (with s as (
                      select substring(pg_get_functiondef(f.f_rows)
                                       from 'pv-mask-begin(.*)pv-mask-end') as x
