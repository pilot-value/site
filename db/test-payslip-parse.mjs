...
console.log('\n③-b 金額の読み方（小数点とけた区切り）');
{
  /* ★ここが唯一の防波堤（2026-09-11）。
     ...
     ★2026-09-15、オーナー決定で**カンマは常に桁区切り**になった（画面もサーバも）。
       区切りが2種類（1.234,56 / 8.450,00）の枝は今までどおりで、上の 1/1000 は
       今も止まる。変わったのは**カンマしか無い**形だけ。
       ⚠️ その代わり、カンマだけで小数を書いた印字（1 234,56 / 1000,50）は100倍に
          読む。これは承知のうえの交換（オーナー判断）。 */
  // [印字, 通貨の小数桁, 期待]
  const M = [
    ['1,234.56', 2, 1234.56], ['1.234,56', 2, 1234.56], ['8.450,00', 2, 8450],
    ['1,150,000', 0, 1150000], ['1.234.567', 2, 1234567],
    ['1,5', 2, 15], ['1000,50', 2, 100050], ['0.500', 2, 0.5], ['1.0', 2, 1],
    ['1,23,456', 2, 123456], ['12.34.56', 2, null], ['', 2, null], [null, 2, null],
    [12345, 2, 12345], ['1234', 0, 1234], ['０', 0, 0],
    /* ★2026-09-15 の規則そのもの。200,000 から 0 を1つ消した形が、
       「読めません」でも 200 でもなく 20,000 になる。 */
    ['200,00', 0, 20000], ['200,00', 2, 20000], ['1,100,00', 2, 110000],
  ];
  ...
  for (const [s, want] of [
    ...
    ['1.234,56 EUR', 1234.56], /* ⚠️ 1 234,56 は空白を落とすと「カンマだけ」＝ 2026-09-15 の規則で 123456。
          カンマだけで小数を書く印字を100倍に読む交換を、ここに現物で残す。 */
    ['1 234,56', 123456], ['１２３４', 1234],
  ...
  /* ⚠️ 署名は増える（2026-09-15 に第2引数 dec が付いた）。切り出しは「行頭の }」まで
        なので、readMoney の中に行頭の } を作らないこと（作ると途中で切れて構文エラー）。 */
  const cutR = HTML.match(/\nfunction readMoney\(s(?:,\s*\w+)?\) \{[\s\S]*?\n\}/);
  ...
  const SHARED = [
    ['1,234.56', 2, 1234.56], ['1.234,56', 2, 1234.56], ['8.450,00', 2, 8450],
    ['1,150,000', 0, 1150000], ['1.234.567', 2, 1234567], ['1,5', 2, 15],
    ['1000,50', 2, 100050], ['0.500', 2, 0.5], ['1.0', 2, 1], ['1234', 0, 1234],
    ['1,23,456', 2, 123456], ['12.34.56', 2, null],
    ['1.000', 0, 1000], ['1.000', 3, 1], ['500.000', 2, 500000], ['500.000', 3, 500],
    /* ★2026-09-15 の規則（カンマは常に桁区切り）。小数の無い通貨の行を足してある ──
       これが無いと「サーバだけ直し忘れた」に気づけない。 */
    ['200,00', 0, 20000], ['200,00', 2, 20000], ['1,100,00', 2, 110000],
    ['3500,50', 2, 350050], ['500.000', 0, 500000],
  ];
