...
/* ════════════════════════════════════════════════════════════════
   ...
   「出すとこの称号が入る」は、マイページの沈んだ板が黙って伝える。
...
/* ════════════════════════════════════════════════════════════════
   ...
   **書き直さない。** 原稿から変えたのは次の2つだけで、どちらも理由がある。
   ...
   update（海外には英語だけ）と違う。海外在住の日本人パイロットに
   読めない1通を送らないための判断で、renewalLangOf がその1か所。
...
/* ★このメールだけ英語が上・日本語が下（2026-09-24 オーナー指示
   「日本人には日本語、それ以外には英語と日本語で」の語順そのまま）。
   これを受け取るのは日本の会員**以外**＝まず英語で読む人なので、英語を先に置く。
   realpay / update の ['ja','en'] とは向きが逆。UPDATE_BOTH_ORDER を共有せず
   別に持っているのはこのため（あちらは過去に送った1通の記録でもある）。 */
const RENEWAL_BOTH_ORDER = ['en', 'ja'];

/* ★送り分け（2026-09-24 オーナー指示）。

     日本の会員 …………………… 日本語だけ
     それ以外の全員 …………… 英語と日本語を1通に（海外と分かる人も、手がかりが無い人も）

   ⚠️ **updateLangOf をそのまま使わない。** あちらは海外と分かる人に英語**だけ**を送る。
      ここで日本語も足すのは、海外在住の日本人パイロットが読めない1通を受け取らないため
      （居住国が海外でも日本語話者である確率が高い、というのがこのサービスの実態）。
      判定の入口だけは updateLangOf を通す ── 日本の会員を見分ける条件を2本に割らない。 */
export function renewalLangOf(p) {
  return updateLangOf(p) === 'ja' ? 'ja' : 'both';
}
