...
### 現状（2026-08-10 に Gmail の設定画面で実測）

- ✅ `PILOT VALUE <info@pilot-value.com>` は**既に登録されている。**
...
- ❌ **「デフォルトの返信モード」が「常にデフォルトのアドレスから返信する」になっている。**
  送信元を用意してあるのに、**返信は毎回 個人アドレスから出る。**ここが唯一かつ致命的な穴。
...
届いたら → ︙ → メッセージのソースを表示 → 元のメッセージをダウンロード
```sh
node mail-bot/check-reply-headers.mjs ~/Downloads/保存した.eml
```
`9/9 passed` なら合格。落ちたら表示された項目だけ直す。
