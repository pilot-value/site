...
- ★**「REAL PAY の画面に出る数」の節（3-c）が、画面の3枚のカードと DEEP PAY の分子を出す**（2026-08-25）。
  ...
  返るのは航空会社・職位・出典・投稿時期の4つだけで、金額・機種は入っていない。
...
| `assert-pay-rows.mjs` | REAL PAY の7つの約束（Give → Get・準識別子は粗い段だけ・有効数字2桁・1行＝1人…）＋**行を押すと出る面**（帯の両端が刻みの倍数・％も生の額も出ない・押してもサーバへ投げない）＋**行に出る年数は昇格後だけ**（在籍年数は返さない）|
...
7. **年数の欄は2つあり、混ぜない**（2026-09-16・オーナー指摘）。
   ...
   **REAL PAY の行に出るのは昇格後年数だけ**（`ten`＝5年幅の5段・0〜4）。
   在籍年数は行に出さない ── 行の並びが `機長 · B787 · …年` なので、読む側は必ず
   「昇格してから」と読む。自社養成の人は昇格した年でも「10〜20年」と出ていた。
   ⚠️ **段を職位ごとに変えない**（刻みの形そのものが職位を指す）。
   ⚠️ **`rank_years > seniority_years` は正しい**（他社から移ってきた機長）。列をまたぐ CHECK を置かない。
   ⚠️ **古い行は `rank_years` が null ＝ 札が出ないのが正解。在籍年数で埋め合わせない。**
   式は [db/pay-rows.sql](db/pay-rows.sql) の `ten` が正・[pay-wizard.js](pay-wizard.js) が写し。
