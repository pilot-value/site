...
// ── ④c 組合が総支給の外で払った分も積む（2026-09-02）──────────
console.log('\n④c 組合が総支給の外で払った分も累計に入る');
{
  /* 本番で実際に起きた形（乗員代表）。会社の明細に印字された総支給と、
     組合から直接受け取った額が別々に書かれている。

     ★累計は「これまでに受け取った額の足し算」なので、組合の分も入れる。
       入れないと、この人の累計が受け取った額の**半分**で止まる
       （オーナー指摘 2026-09-02）。年収（pv_annual_total）が組合を
       足しているのに累計だけ足していない、という食い違いでもあった。
     ★控除の分母（netGross）には**足さない**。組合の分は会社の明細の外にあり、
       そこから引かれた控除が1円も無い。足すと画面の
       「額面 − 手取り ＝ 控除」が組合の額ぶんズレる。 */
  const t = V.totals([mk(2025, 1, {
    gross_monthly: 60000, union_pay: 40000, union_outside_gross: true
  })]);
  ok(near(t.gross, (60000 + 40000) * 40),
     '★ 累計＝（総支給 ＋ 総支給の外の組合分）×レート', String(t.gross));
  ok(near(t.unionOut, 40000 * 40),
     '★ そのうち組合の分を内数で返す（画面が1行で断れるように）', String(t.unionOut));
  ok(near(t.deduct, 10000 * 40),
     '★ 控除は会社の明細のぶんだけ（組合を足すとここがズレる）', String(t.deduct));
  ok(near(t.net, 50000 * 40), '手取りは今までどおり', String(t.net));

  /* 支給元が会社（union_outside_gross が偽）なら、その額は総支給の中にある。
     二重に足さない。 */
  const inn = V.totals([mk(2025, 1, {
    gross_monthly: 60000, union_pay: 40000, union_outside_gross: false
  })]);
  ok(near(inn.gross, 60000 * 40),
     '★ 会社が払っている組合手当は二重に足さない', String(inn.gross));
  ok(inn.unionOut === 0, '★ そのとき内数は 0');

  // 折れ線（running total）も同じ額で伸びる。ここだけ別の式にしない
  ok(near(t.series[0].gross, (60000 + 40000) * 40),
     '★ 積み上がりの折れ線も同じ額', String(t.series[0].gross));
}
