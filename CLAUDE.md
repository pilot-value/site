...
- ★**「REAL PAY の画面に出る数」の節（3-c）が、画面の3枚のカードと DEEP PAY の分子を出す**（2026-08-25）。
  ...
  2026-09-18 から、上の8行だけ1人に固定の型ごとに欄が出る（年収型は年収・年数の段／
  機種型は職位・機材・年収／会社型は会社・職位）。9行目以降は会社と投稿時期
  （上の8行から下がってきた人は投稿時期だけ・公開前から9行目以降にいた人はずっと会社型）。
  会社と年収が同じ行に入ることは無い。内訳・勤務は今も入っていない
  （詳細は [workflows/deploy-checklist.md](workflows/deploy-checklist.md) の「鍵の無い人の一覧」）。
  ★2026-09-19 から、同じ会社・同じ職位の公開年収から大きく外れた**本人申告**の行は、開いた一覧で ⚠ が付き、
  伏せた一覧の上の8行には入らない（数字は書き換えない・集計にも今までどおり入る）。
