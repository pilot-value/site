...
19. `node assert-pay-rows.mjs` — **REAL PAY**（[actual-pay.html](actual-pay.html)）の検査。
   ...
   **後ろ2つはまだページが無い。`<span aria-disabled="true">` で置き、`href` を書かない**
   ...
   ④1行＝1人（同じ人の複数月は年換算の中央値で1行）⑤引数ゼロ ⑥並びに時間が無い（`md5(人のキー)` 順）
   ⑦常識の幅（年 $10,000〜$700,000 の外は打ち間違いとして出さない）。
   ...
   **ゆるめたのはここ1か所だけ。⑥は触っていない**（並びは今も `md5(人のキー)` 順）。
   **同じ会社・同じ職位の同僚には当てられうる**水準であることは承知のうえで選んだ形。見るのは:
   ...
   - 基地コード・在籍年数・年代・**投稿の日付**・原本通貨・契約形態・国籍・`proof_hash`、
     そして**自由入力で打ち込まれた社名**が1つも出ない（`airline:'other'` は固定の札に置き換える）。
   ...
   - **並び替えの口を作らない**（`sort` / `新しい順` / `order` の select が日英の HTML と JS に無い）。
     ⚠️ これは契約⑥に真っ向から反する。並びに投稿の新しさが乗ると、**誰が最近出したか**が読める
   ...
   - ★**いちばん右に「投稿時期」の列**（2026-08-24 オーナー指示「投稿の日付を入れよう。
     ...
     - **時期で並べ替える口も、時期で絞る口も作らない。**⚠️ ここが契約⑥の急所で、
