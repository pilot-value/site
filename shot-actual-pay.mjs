...
/* 作り物の行。★ana / jal は実在の会社だが、この金額は作った数字。
   ...
   ★2026-09-03、機材・在籍の段・報酬の内訳・勤務が返るようになった（行を押すと出る面の中身）。
     2026-08-24 に「返さない」と書いてあったのはこの日に取り消した。
       ...
       ten  … **年数**の段だけ（2026-09-16 に昇格後年数を第一へ差し替えた）。
              0〜4 の5年幅（5年未満 / 5〜10 / 10〜15 / 15〜20 / 20年以上）で、
              **職位では分けない**（fo も cap も同じ5段）。年そのものは来ない
       tenk … その段をどちらの年数で作ったか（r＝昇格後／s＝在籍）。
              昇格後年数の欄は 2026-09-16 に作ったので、それより前の投稿は
              在籍年数で段を作って出す（オーナー指示「これまで提出してもらった
              ものは今まで通り出して」）。画面は「昇格後10〜15年」と
              「在籍10〜15年」を別の言葉で出すので、**絵にも両方入れてある**
...
function checkRows(list, tag) {
  ...
  list.forEach((r, i) => {
    ...
    if (typeof r.ten === 'number' && (r.ten < 0 || r.ten > TEN_MAX))
      bad.push(at + ' 年数の段 ' + r.ten + ' は 0〜' + TEN_MAX + ' の外');
    if (r.tenk != null && r.tenk !== 'r' && r.tenk !== 's')
      bad.push(at + ' tenk ' + r.tenk + " は r / s のどちらでもない");
    if (r.tenk != null && typeof r.ten !== 'number')
      bad.push(at + ' 段が無いのに tenk だけ持っている');
...
const ROWS = [
  R('ana', 'fo', 110000, false, 0,                  // 預かり
    { fleet: 'a320', ten: 0,
      pay: P({ base: [70000, 75000], variable: [25000, 30000],
  ...
  R('air-canada', 'cap', 240000, false, 0,          // 預かり
    { fleet: 'b777', ten: 1,
      pay: P({ base: [140000, 150000], variable: [50000, 60000],
  ...
  R('ana', 'fo',  95000, false, 1,
    { fleet: 'a320', ten: 0,
      pay: P({ base: [60000, 65000], variable: [20000, 25000],
               perdiem: [5000, 10000], bonus: [0, 10000] }),
      work: WK([50, 60], [12, 14], [14, 16]) }),
  /* ★昇格後年数の欄ができる前の投稿（在籍年数で段を作った行）。
       札が「在籍5年未満」になっていることを絵で確かめるために1行だけ置いてある。 */
  R('jal', 'fo',  99000, false, 1,
    { fleet: 'b767', ten: 0, tenk: 's',
      pay: P({ base: [65000, 70000], variable: [20000, 25000],
...
const MANY = [
  ...
  R('jal', 'cap', 185000, true, 2, { fleet: 'b787', ten: 1, tenk: 's' }),  // 古い投稿＝在籍年数の段
...
/* ★行を押して出る面のための10行（2026-09-03）。
     ⚠️ 10行までにしてあるのは、**1ページに全部載せるため**（10件で改頁するので、
        2ページ目の行は押せない ＝ 撮れない）。
     並びは新しい順。上の7行が給与フォーム由来（段0）、8行目が古い投稿（段1）、
     下の2行が口コミ由来（段1）。
     row= で選ぶ行の見え方：
       ...
       2 勤務だけ（内訳の節が丸ごと出ない）
       4 機材と年数の札はあるが年収だけ
       7 ★昇格後年数の欄ができる前の投稿。札が「在籍◯年」になる（2026-09-16）
