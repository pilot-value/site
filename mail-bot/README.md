...
```
{{ .Token }} — Login code / ログインコード
```

件名に6桁を入れてあるのは、**スマホの通知バナーだけ見てブラウザに戻れるようにするため。**

★2026-09-02、英語を先に置き直した。テンプレートは相手の言語で出し分けられない
（Supabase のテンプレートは1枚しか無い）ので、どちらかが先に来る。
先頭は6桁のコードで言語に関係なく読めるから、その後ろは
**日本語の読み手には Login code が通じるが、英語の読み手には「ログインコード」が読めない**
という非対称で決めた。本文（`auth-emails/signin-code.html`）の順は日本語が先のままで、
ここだけ入れ替えている（本文は2行あって両方読めるので、順の非対称が効かない）。
...
- **★送り分けは2つだけ**（2026-09-24 オーナー指示）。
  日本の会員は**日本語だけ**・**それ以外は全員 英語と日本語を1通に**（英語が上・日本語が下）。
  `update` は海外に英語**だけ**を送るが、このメールは日本語も付ける
  ―― 海外在住の日本人パイロットが読めない1通を受け取らないため。判定は `renewalLangOf` の1か所。
...
- **投稿があった航空会社は1件でも名前を出す**（2026-09-24 オーナー指示）。
  **★代わりに件数を書かない** ―― 「◯◯社 1件」と書くと、その週にその会社から出した
  **たった1人**が居ることまで伝わる。名前だけなら1人の週と4人の週が同じ見た目になる。
  （サイトは元から会社を公開しているが、時期の粗さは「1ヶ月以内」まで。
  `db/pay-rows.sql` の `age`。メールは週なので、件数まで足すとそこだけ細かくなる。）
  並べるのは多い順に最大12社、超えたぶんは「ほか◯社」（`DIGEST_NAME_MAX`）。
