/* ════════════════════════════════════════════════════════════════
   ...
   **1つも返ってこない**（db/pay-rows.sql が出さない）。
   ...
   ★2026-09-03、オーナー判断で機材・年数・内訳・勤務を出すことにした。
     ...
     ★年数は「5〜10年」のような段だけ。年そのものはサーバから来ない。
     ★2026-09-16、その段の中身を**在籍年数から昇格後年数へ替えた**（オーナー指摘）。
       段は職位のすぐ隣に出るので「昇格して何年目」と読まれるのに、中身は
       入社何年目だった（自社養成は訓練の年も入る）＝入ったばかりの機長が
       「10〜20年」と出ていた。刻みも5年幅の5段にそろえ、職位で分けるのをやめた。
   ...
   **もう一度2桁に丸め直す**（$180,000 → ¥2,861万 を ¥2,900万 に）。
   ...
   ★行は押せる。押すと右から**その1人ぶんの面**が出る（ページ遷移しない）。
     出るのは 機材 / 昇格後年数の段 / 報酬の内訳の帯 / 勤務の帯 / 同社同職位の他の記録。
...
(function (w, d) {
  ...
  var T = {
    ja: {
      ...
      lockPC: '給与の内訳を入力する',
      /* 年数の段。★年そのものは持っていない（サーバが 0〜4 の段だけを返す）。
         ★2026-09-16、ここは在籍年数の段で、職位ごとに刻みが違った。
           職位のすぐ隣に出るので「昇格して何年目」と読まれる＝画面が嘘をついていた。
           いまは5年幅で5段、職位で分けない（オーナー指示）。
         ★いちばん下は「5年未満」。「1〜5年」と書かないこと ── 今年上がった人は
           0 か 1 と答えるので、同じ嘘をもう一度作ることになる。
         ★札は**2組ある**（2026-09-16・オーナー指示「これまで提出してもらった
           ものは今まで通り年数を出して」）。昇格後年数の欄はこの日に作ったので、
           それより前の投稿は在籍年数から段を作って出す。サーバは tenk で
           どちらかを必ず伝えてくる（r＝昇格後／s＝在籍）。
           ⚠️ **1組にまとめないこと。** まとめた瞬間、古い行が「昇格後20年以上」と
              名乗り直し、この日直した嘘がそのまま戻る。 */
      ten: { r: ['昇格後5年未満', '昇格後5〜10年', '昇格後10〜15年',
                 '昇格後15〜20年', '昇格後20年以上'],
    ...
    en: {
      ...
      dwOpenM: 'Open the record at {air}',
      dwClose: 'Close',
      ...
      ten: { r: ['Under 5 yrs in rank', '5–10 yrs in rank', '10–15 yrs in rank',
      ...
      mkS: 'Share one pay record to see the annual figures, rank and aircraft',
      mkA: 'Annual figure hidden',
      mkF: 'Rank and aircraft hidden',
  ...
  var S = {
    ...
    /* 'preview' … 作り物の5行（ap-preview.js）。サーバが行を返さないとき
       'masked'  … **本物の行**。ただしサーバが会社・出典・投稿時期しか
                    返していない（年収・職位・機材は渡ってきていない）
       'open'    … 年収も機材も入っている
       'error'   … 読めなかった */
  ...
  /* ★伏せた一覧（2026-09-16）。**プレビューと同じ扱いにしない。**
       ...
       こちらは本物の投稿で、オーナーが出すと決めた3つ（会社・出典・
       投稿時期）はそのまま読める。 */
  ...
  /* 年数の段（2026-09-03。2026-09-16 に昇格後年数を第一へ差し替え）。
       ★サーバが返すのは 0〜4 の**段だけ**で、年は来ない。
       段の外（古いサーバ・想定外の値）は**空**にする。
       ここで「不明」の札を作らない ── 「書いていない人」という情報も出さない。
       ★職位で引き当てない。段の意味は職位に依らず同じ（5年幅）。
       ★どちらの年数で作った段かは tenk が決める（r＝昇格後／s＝在籍）。
         tenk が来ない古いサーバは昇格後年数だけで段を作っていたので 'r' に倒す。
       ⚠️ tenk を無視して片方の札だけで出さないこと ── 在籍年数から作った段に
          「昇格後」と書いた状態が、2026-09-16 に直した嘘そのもの。 */
  function tenName(r) {
    if (!r || typeof r.ten !== 'number') return '';
    var k = (r.tenk === 's') ? 's' : 'r';
    return T.ten[k][r.ten] || '';
  }
  ...
  function visibleRows() {
    ...
    return S.rows.filter(function (r) {
      ...
      /* ★伏せた一覧では職位を受け取っていないので、職位で絞らない（2026-09-21）。
         ⚠️ isMasked() を外さない。外すと ?pos=… を付けた住所で来た人の一覧が
            1行残らず消える（選べる口は隠してあるが、住所は手で打てる）。 */
      if (S.fPos && !isMasked() && r.pos !== S.fPos) return false;
      return true;
    });
  }
  ...
  /* › のボタンの読み上げ。★社名と職位だけ。金額を読ませない。
     ★伏せた一覧では職位を受け取っていないので社名だけで読む（2026-09-21）。
     ⚠️ replace の新しい側は関数で渡す（社名に $ が入りうる）。 */
  function openLabel(r) {
    if (isMasked()) {
      return T.dwOpenM.replace('{air}', function () { return airName(r.airline); });
    }
    return T.dwOpen
  ...
  /* ★公開年収から大きく外れた本人申告の行（2026-09-19・オーナー指示）。
     ...
     ★記号は**文字**で描く。svg にすると表の中の svg が1つ増えて、
       「棒グラフを描き直した人が居ないか」を見ている検査に当たる。
     ⚠️ 伏せた一覧とプレビューには出さない。サーバが伏せた行に far を入れないのが
        1本目の鍵で、ここでも画面の種類で止めるのが2本目。 */
  ...
  function dwHTML(r) {
    ...
    var mk = isMasked();
    /* ★伏せた面では職位も受け取っていない（2026-09-21 オーナー指示）。
         空から始めて、下の板1枚だけがこの行に残る。 */
    var meta = mk ? [] : [posName(r.pos)];
    if (!mk) {
  ...
  function openDrawer(i, trigger) {
    ...
    /* ⚠️ 残課題（2026-09-07・Phase 4 より前からある）── 広い画面で
         **一覧をいちばん下まで送った状態**で面を開くと、閉じたあとの位置が
         約 9px 下へずれる。ここで縦の巻き取りが消える → 本文の幅が広がって
         組み直され、いちばん下の位置そのものが変わるため。
       ★戻るで閉じたときも × で閉じたときも**同じだけずれる**（実測）。
         ＝ ブラウザの「戻る」を足したことが原因ではない。
         狭い画面（巻き取りが元から重ならない）では起きない。
       直すなら overflow を止める代わりに巻き取りぶんの余白を足す形にするが、
       画面の見た目に触るので Phase 6 の担当。 */
  ...
  function wipeReal(client) {
    ...
    /* ★捨ててから取り直す（2026-09-16）。この順を入れ替えない ──
         ...
         取り直して返るのは伏せた行（会社・出典・投稿時期の3つ）だけ。
  ...
  function maskedList() {
    ...
    for (var i = 0; i < page.length; i++) {
      ...
      h += '<tr class="ap-r ap-r--mk" data-ap-row="' + esc(String(r._i)) + '">'
         ...
         /* ★職位も機材も、サーバが値を渡していない（2026-09-21 オーナー指示で
              職位も送るのをやめた）。値の有無で分岐せず、無条件に板を1枚置く。
            ⚠️ ここで posName(r.pos) を呼び戻さない。呼べば「受け取っていない物を
               画面が書く」ことになり、伏せた一覧の約束（ぼかしではなく渡さない）が崩れる。 */
         + '<td><span class="ap-flt ap-flt-mk">'
         +     '<span class="ap-flt-lk" aria-hidden="true"></span>'
         +     '<span class="ap-pv-sr">' + esc(T.mkF) + '</span></span>'
         + '</td>'
         + '<td class="ap-num"><span class="ap-cl" aria-hidden="true">' + esc(T.thAmt) + '</span>'
  ...
  function renderFilters() {
    ...
    /* ★伏せた一覧は職位を受け取っていない（2026-09-21 オーナー指示）。口だけ残すと
         空のプルダウンが1つ並び、「選べるのに必ず0件」になる。枠ごと隠す
         ── 帯と枠の両方に [hidden] が要るのは上の .ap-filter と同じ理由。 */
    var pSel = el('ap-pos'), pBox = pSel && pSel.closest && pSel.closest('.ap-f');
    if (pBox) pBox.hidden = isMasked();
    if (isMasked()) { S.fPos = ''; return; }
  ...
  function boot() {
    ...
    /* ── ブラウザの「戻る」──────────────────────────────────
       ...
       ★開いていなければ、原則**何もしない** ── 詳細と関係のない普通の移動を
         妨げない。例外は「再読み込みで取り残された段」の1つだけ（下）。 */
    w.addEventListener('popstate', function () {
      if (DW) { closeDrawer(false, true); return; }
      /* ★開いたまま再読み込みすると、同じ URL の段が1つ余る（履歴から段を消す
           手立ては browser に無い）。開き直せば ① が引き継ぐが、**開かずに戻ると
           その余った段が1回ぶん吸ってしまい、一覧に留まる**（2026-09-07 に実測）。
           吸われたぶんをそのまま次へ送る。★送るのは1回だけ。
           余った段と1つ前の段は**同じ URL の一覧**なので、飛ばしても見えている
           画面は変わらない。 */
      if (DWO) {
        DWO = false;
        if (w.history && w.history.back) { try { w.history.back(); } catch (e) {} }
      }
    });
  ...
  function fetchRows(client) {
    ...
    Promise.resolve(client.rpc('pv_pay_rows')).then(function (res) {
      ...
      if (!isOpen) {
        /* ★鍵が無い。サーバが返すのは**伏せた行**（会社・出典・投稿時期の
             3つだけ。年収も職位も機材も入っていない）。
    ...
    ready.then(function (session) {
      if (!session) {
        /* ★未ログイン。2026-09-16 にオーナー判断で**一覧を取りに行く**ことにした。
             サーバが返すのは会社・出典・投稿時期の3つだけの行で、
             年収も職位も機材も渡ってこない（db/pay-rows.sql の mask）。
