...
--
--     ① 鍵         給与明細を1枚出した人だけ・90日（サーバ側）。
--                   鍵の無い人に返すのは、型ごとの欄だけを入れた伏せた一覧
--                   （下の「★鍵の無い人の一覧について」）
--     ② 準識別子は「粗い段」だけ
...
--
-- ★鍵の無い人の一覧について（2026-09-16 に作り、2026-09-18・09-19 に作り直した）
--   給与をまだ1件も出していない人（未ログイン・会員登録だけ・口コミだけ）にも
--   **本物の一覧**を返す。ただし行に入れるのは、下の型で決めた欄だけ。
--   **ぼかすのではなく、渡さない。** 画面は渡らなかった欄に空の板を描く。
--
--   ・上の8行   … 機長と副操縦士を交互に（機長が先）。それぞれの中は新しい順。
--                  片方が4人に満たなければもう片方で埋める。訓練生などは入れない。
--                  ★2026-09-18 オーナー指摘。新しい順だけだと、直近の提出が
--                    副操縦士に偏った週は上が全部副操縦士になり、出すか迷っている
--                    機長が「機長は出していない」と読んでためらう。
--   ・型         … 1人に1つ、本人の匿名キーから決める（読み込むたびに変えない）。
--                    年収型 … 年収（有効数字2桁）・年数の段（昇格後／在籍の札つき）
--                    機種型 … 職位・機材・年収（2026-09-19 に年収を足した）
--                    会社型 … 会社・職位（年収は出さない）
--                  8行には、どの型にも出典と投稿時期を添える。
--   ・9行目以降 … 会社と投稿時期だけ（2026-09-19 オーナー指示）。
--                  ただし**上の8行から下がってきた人**（会社型を除く）は投稿時期だけ。
--                  8行で年収や機材を見せた人の会社を、前後の画面の見比べでつながせない。
--   ・公開の前から9行目以降にいた人（＝もう会社が出ている人）は、出し直して
--     8行に上がっても会社型で出す。本来の型で出すと、消えた会社と増えた年収がつながる。
--     ＝ **会社と年収が同じ行に並ぶことは無く**、1人の上でもつながらない。
--     見分け方と残る穴は mask の節の冒頭。
--   ・預かり（登録前に出されたぶん）は8行に入れず、投稿時期だけ。
--     引き取られるとキーが変わり、型が引き直されるため。
--   ・内訳・勤務は、どの型にも入れない。
--
--   ★正直に書いておく ── 年収型の行は職位を板にするが、8行は機長から始まる
...
--   ・鍵の無い人の一覧（mask）に、型に無い欄・内訳・勤務を足すこと。
--     会社と年収を同じ行に入れること。型を読み込むたびに引き直すこと。
--     （2026-09-16 から anon にも execute を渡している。守りは「何を返すか」の側）
...
--   ★違いは rows だけ。鍵が無い人の行は、型ごとの欄だけを入れた伏せた行
--     （2026-09-16 から。形は下の mask とファイル冒頭「★鍵の無い人の一覧について」）。
--
...
begin
  ...
  person as (
    ...
    select pkey, airline, pos,
           ...
           --   並び順と右端の列が食い違って見える。
           max(cat) as last_at,
           /* ★鍵の無い人の一覧（mask）だけが読む時刻。**行には入れない**。
                公開の日（2026-09-19）より前の、その人のいちばん新しい提出。
                公開前から一覧にいた人を見分け、公開の日の「上の8行」を並べ直すのに使う
                （理由は mask の節の冒頭）。
              ⚠️ この日付を後ろへずらさないこと。ずらすと公開の日の8行が並べ直され、
                 8行で年収を見せた人の会社が9行目以降に出る。 */
           max(cat) filter (where cat < timestamptz '2026-09-19 00:00:00+09') as l0
      from sane
  ...
  coarse as (
    /* ── 行に出してよい粗い段だけを持つ節（2026-09-18 に listed から切り出した）──
       ★本物の一覧（listed）と伏せた一覧（mask）の**両方がここを読む**。
         段の式を2本にしないため。2本あると片方だけ直して、同じ人が
         画面ごとに違う段に出る。
       ★持つのは機材のコードと年数の段（ten / tenk）だけ。生の年数・内訳・勤務は
         ここへ持ち込まないこと ── mask はこの節を読んでよいことになっている。
       ...
       **職位で段を変えない。** 上端だけ開いていて、あとは同じ幅。
       ...
       ★キー名は ten（tenure の頭3文字）。年数を表す英単語を
         ...
         （注意書きのつもりで書くと注意書きだけで赤くなる）。 */
    select k.pkey, k.airline, k.pos, k.fleet,
           case when k.pos not in ('fo','cap') then null
  ...
  listed as (
    ...
    select coalesce(jsonb_agg(jsonb_strip_nulls(jsonb_build_object(
             ...
             'fleet',      k.fleet,
             /* 年数は**段だけ**。式と理由は上の coarse（段の式はあそこ1か所だけ）。
                ★tenk（どちらの年数で作った段か）を落とさないこと ── 落とすと
                  古い行が「昇格後20年」と名乗り直し、2026-09-16 に直した嘘が戻る。 */
             'ten',        k.ten,
             'tenk',       k.tenk,
  ...
  mask as (
    ...
    /* ── 鍵の無い人へ返す行（2026-09-16 オーナー指示／2026-09-18・09-19 作り直し）─
       ★件数は本物と同じ（同じ person）。行を抜き差ししない
         ── 数え上げのカードと表の行数が食い違う。
       ★**上の8行**は機長と副操縦士を交互に並べる（機長が先）。
         どちらかが4人に満たなければ、もう片方で埋める。訓練生などは入れない。
         同じ職位の中は契約⑥の順のまま（新しい順・同じ時刻は md5）。
         9行目以降は、残りを新しい順に並べる。
         ★2026-09-18 オーナー指摘「最初の画面は FO しかいない」。新しい順だけだと、
           直近の提出が副操縦士に偏った週は上が全部副操縦士になり、
           出すか迷っている機長が「機長は出していない」と読む。
       ★**型は1人に1つ、ずっと同じ。** 本人の匿名キーから決める。
         読み込むたびにくじを引かないこと ── 引くと、読み直して見える欄を
         足し合わせれば1行が丸ごと読める。
         塩（'pv-tz:'）は並びに使う md5 と別にしてある（並びと型の相関を切る）。
           年収型 a … 年収（有効数字2桁）と年数の段（ten / tenk）
           機種型 f … 職位・機材・年収（2026-09-19 オーナー指示で年収を足した）
           会社型 c … 会社と職位（年収は出さない）
         上の8行には、どの型にも出典と投稿時期を添える。
       ★**9行目以降は会社と投稿時期**（2026-09-19 オーナー指示
         「会社名を出すが、上から下がってきた人には出さない」）。職位・出典は出さない。
         会社を出さないのは次の人だけ（＝投稿時期だけ）：
           ・上の8行から下がってきた人（会社型を除く）。8行で年収や機材を見せた人の
             会社をここで出すと、前後の画面を見比べて1人の会社と年収がつながる。
             「8行にいたか」は記録していないので、次のどちらかで決める：
               公開の日より後に出した人 … 出した瞬間にいちばん新しい＝8行に入る
               公開の日に8行にいた人   … 公開前の提出だけで8行を並べ直して決める
           ・預かり（キーが p: で始まる行）。8行にも入れない。引き取られた瞬間に
             キーが変わる＝型が引き直されるので、前後を見比べると2つの型が1人に重なる。
       ★**公開の前から9行目以降にいた人は、ずっと会社型として扱う**（型の欄が c になる）。
         この人たちは公開の時点で会社が出ている。出し直して8行に上がったとき本来の型
         （年収型・機種型）で出すと、「9行目以降から消えた会社」と「8行に増えた年収」が
         1人につながる。会社型なら8行に上がっても会社と職位だけで、新しく分かることが無い。
         ⚠️ 残る穴（どちらも「給与を1件出した人なら最初から全部見える」範囲の話）：
            ・同じ会社・同じ月の出し直しは投稿時刻を上書きする（pay_reports_uniq）。
              公開前の提出がそれ1件だけの人が訂正すると「公開後の人」に変わり、
              公開の日の8行の並べ直しも1人ずれる。
    ...
    select coalesce(jsonb_agg(jsonb_strip_nulls(
             case when q.top and q.tt = 0 then jsonb_build_object(
                    'annual_usd', public.pv_sig2(q.v),
                    'ten',        q.ten,
                    'tenk',       q.tenk,
                    'verified',   q.verified,
                    'age',        q.age,
                    't',          'a')
                  when q.top and q.tt = 1 then jsonb_build_object(
                    'annual_usd', public.pv_sig2(q.v),
                    'pos',        q.pos,
                    'fleet',      q.fleet,
                    'verified',   q.verified,
                    'age',        q.age,
                    't',          'f')
                  when q.top then jsonb_build_object(
                    'airline',    q.airline,
                    'pos',        q.pos,
                    'verified',   q.verified,
                    'age',        q.age,
                    't',          'c')
                  -- 9行目以降。会社型・公開前から会社が出ている人・8行に入らない職位は会社も
                  when not q.pend and (q.tt = 2 or not q.eli) then jsonb_build_object(
                    'airline',    q.airline,
                    'age',        q.age)
                  else jsonb_build_object(
                    'age',        q.age)
             end)
             -- 8行（職位ごとの番号 → 同じ番号なら機長が先）→ 残り（新しい順）
             order by q.top desc,
                      case when q.top then q.rn else q.g end,
                      q.pos = 'fo'), '[]'::jsonb) as j
      from (
        select z.*,
               ...
               /* 見せる型。公開前からいて、公開の日の8行に入っていなかった人は会社型。
                  ⚠️ 下の rn0 / qcap0 は**公開の日の8行を並べ直すためだけ**の式。
                     ...
                     変えると公開の日の8行が別の顔ぶれになり、年収を見せた人の会社が出る。 */
               case when z.l0 is not null
                     and not (z.eli0 and z.rn0 <= case when z.pos = 'cap' then z.qcap0
                                                       else least(z.nfo0, 8 - z.qcap0) end)
                    then 2 else z.t end as tt
          from (
            select y.*,
                   -- 機長の枠。副操縦士が4人に満たなければ、その分だけ機長を増やす
                   least(y.ncap, greatest(4, 8 - y.nfo)) as qcap,
              ...
              from (
                select x.*,
                       ...
                       count(*) filter (where x.eli0 and x.pos = 'cap') over () as ncap0,
                       count(*) filter (where x.eli0 and x.pos = 'fo')  over () as nfo0
                  from (
                    select p.pkey, p.airline, p.pos, p.v, p.verified, p.age, p.last_at, p.l0,
                           k.fleet, k.ten, k.tenk,
  ...
  select jsonb_build_object(
           ...
           -- ★行はここだけで落とす。鍵が無ければ mask の行（型ごとの欄だけ。
           --   8行の外は会社か投稿時期だけ）に差し替わる。型に無い欄は**渡さない**ので、
...
--   ★2026-09-18 から、上の8行には型ごとに年収・機材・年数の段のどれかが入る
--     （ファイル冒頭「★鍵の無い人の一覧について」）。会社と年収が同じ行に並ばない
--     こと・型が人ごとに固定であることで、1行を1人に当てさせない。
--     内訳・勤務は今も mask が**そもそも組み立てない**。
...
comment on function public.pv_pay_rows() is
  ...
  '2026-09-16 から未ログイン（anon）でも呼べる。鍵の無い人に返る行は型ごとの欄だけ'
...
from (
  ...
  --   下の65（型ごとの欄だけ）と66（渡っている相手は2つだけ）が持つ。
  ...
  --   mask が組んだ型ごとの欄だけの行で、内訳も勤務も入っていない（中身の検査は65）。
  ...
  select 65, '★伏せた行は型ごとの欄だけ（内訳・勤務は組み立てもしない・会社と年収は同じ行に無い）',
         /* mask の節だけを印で切り出して見る。関数全体を見ると、本物の行を
            組む listed 側の語に当たって必ず緑になる＝何も守れない。
            ★2026-09-18 に作り直した（上の8行は型ごとに年収・機材・年数の段を出す）。
              許すのは person と coarse（機材と年数の段だけを持つ節）を読むこと。
              内訳・勤務を持つ節（pick / paid / worked / grid / sane）は読まない
              ── 読まなければ、うっかり列を足しても出しようが無い。
            ★型は本人のキーから決める（塩 pv-tz:）。random() や時刻で決めると
              読み直すたびに型が変わり、見える欄を足し合わせて1行が丸ごと読める。
            ★会社と年収が同じ jsonb_build_object に入っていないことも見る。
              組み立てる箇所ごとに切り分けて、両方を持つかけらが1つも無いこと。 */
         case when f_rows is null then false
              else (with s as (
                    ...
                    select s.x is not null
                       ...
                       and s.x like '%pv-tz:%'
                       and s.x not like '%random(%' and s.x not like '%now()%'
                       ...
                       and not exists (
                             select 1
                               from regexp_split_to_table(s.x, 'jsonb_build_object') as g(piece)
                              where g.piece like '%''airline''%'
                                and g.piece like '%''annual_usd''%')
                      from s)
         end from f
  union all
