...
19. `node assert-pay-rows.mjs` — **REAL PAY**（[actual-pay.html](actual-pay.html)）の検査。
   ...
   **同じ会社・同じ職位の同僚には当てられうる**水準であることは承知のうえで選んだ形。見るのは:
   ...
   - 基地コード・在籍年数・年代・投稿月・原本通貨・契約形態・国籍・`proof_hash`、
   ...
   - **列は5つ**（ロゴ＋会社 / 職位 / 年収 / 月あたり / 出典）。見出しは `年収`。
   ...
   - **図は年収の分布の棒1枚だけで、置き場所は表の右横**（オーナー指定。狭い幅では表の下に回る）。
     ★**支給の内訳（ドーナツ）は DEEP PAY へ移した。**この画面は `pay-viz.css` / `pay-viz.js` を
     読まず、`comp` を1文字も持たない。行を押しても何も起きない（`data-ap-row` も無い）。
     見るのは:
     - **棒のところに数字が1文字も無い**（人数は高さで目分量に読めるところまで。オーナー判断）
     - **図に出る金額は軸の両端だけ**（自分の明細の内訳の額が混ざらない）
     - 自分の年収が無ければ「あなたの位置」の破線を出さない（軸の外に置くと目盛りが嘘に見える）。
       破線の位置は [my_pay_reports()](db/pay-reports.sql#L916) 由来で、他人の表から自分を探さない
     - 絞り込むと軸はその中の下端・上端に描き直る（全体のままだと軸が嘘になる）
