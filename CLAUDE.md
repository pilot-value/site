...
19. `node assert-pay-rows.mjs` — 「他のパイロットの実給与」（[actual-pay.html](actual-pay.html)）の検査。
   ...
   ①鍵（明細を1枚出した人だけ・90日・サーバー側）②**内訳は割合だけ**（旧「準識別子ゼロ」） ③有効数字2桁
   ④1行＝1人（同じ人の複数月は年換算の中央値で1行）⑤引数ゼロ ⑥並びに時間が無い（`md5(人のキー)` 順）
   ⑦常識の幅（年 $10,000〜$700,000 の外は打ち間違いとして出さない）。
   **同じ会社・同じ職位・同じ機材の同僚には当てられうる**水準であることは承知のうえで選んだ形。見るのは:
   - **鍵の無い人に金額が1文字も出ない**（サーバー側で止める。画面のモザイクではない）
   - **表は1枚だけ。**粒度を2つに分けた形（機材別＋カテゴリ別）に戻さない
     （同じ人が両方に出て、二重に数えたように見える）
   ...
   - **ページ送りは10件ずつ。出すのは「何ページ目か」だけ**（総件数を出すと会員規模が漏れる）。
     行き止まりが無いこと・絞り込みを触ったら1ページ目に戻ることを総当たりで見る
   - **表の上に図が2枚ある**（支給の内訳のドーナツ / 年収の分布の棒。2026-08-24）。
     ★オーナー判断「一旦やってみよう。イマイチならやめよう」で入れた。**引き返す口がある**（下記）。
     ★これで**②は「準識別子ゼロ」ではなくなった。** 増えた露出は「1人ぶんの支給の内訳」で、
     サーバーが返すのは `comp` ＝ **m/b/d/h/o の整数パーセント5つだけ。金額は1つも返さない**
     （月々の支給 / 年1回の賞与 / パーディアム / 住宅手当 / その他の手当）。
     ⚠️ **画面の年収（有効数字2桁）に掛ければ ±10% ほどで実額は逆算できる。** 承知のうえの形。
     見るのは:
     - **他人のドーナツに通貨記号も桁区切りの数字も1つも出ない**（凡例は「◯%」だけ）。
       ここが崩れると1人ずつの実額がそのまま出る
     - **中央の数字は表に出ている年収そのもの**（別の丸め方をしない）
     - 金額が出るのは**自分の支給構成だけ**で、それは [my_pay_reports()](db/pay-reports.sql#L916) 由来。
       他人の表から自分を探し当てる作りにしない
     - **閉じ方が3つ**（もう一度押す／「自分に戻す」／ESC）。絞り込みを触っても選択は外れる
     - **行を選んでも並びが1行も動かない**（選択が並べ替えに化けない）
     - `comp` が `null` の行を選んでも壊れず、静かに「内訳を出せません」になる
     - **棒のところに数字が1文字も無い**（人数は高さで目分量に読めるところまで。オーナー判断）
     - 自分の年収が無ければ「あなたの位置」の破線を出さない（軸の外に置くと目盛りが嘘に見える）
     - **`pay-viz.css` を `actual-pay.css` より先に読む**（`.pt-*` の詳細度が同じ 0,2,0 で、
       後に書いたほうが勝つ。逆に置くと暗い前提の色が明るい画面に出る）。`pay-viz.js` も同様に先
     - 割合を作る3つ（`pv_pay_comp` / `pv_pct5` / `pv_pending_comp`）は**誰にも grant しない**
     ⚠️ **やめるとき**：サーバーが `comp` を返すのをやめるだけでよい。
     画面は `comp` が無ければドーナツを描かない（自分の内訳と分布は残る）。
     契約ヘッダの②も元の「準識別子ゼロ」に戻す（手順は [db/pay-rows.sql](db/pay-rows.sql) の冒頭）
