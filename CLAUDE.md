...
- 教官・審査・組合などの**追加ロールの手当はここに入れない**（役割ごとのモジュールで別に聞く）。
  ★教官・訓練だけは 2026-08-26 その3 に用意した（下の節）。審査・組合などはまだ無い。
...
- ★**必須は1つも置いていない**（`req-tag` を書かない＝段のゲートも送信の条件も動かない）。
  ...
  （金額・払われ方・単価は「別途支給されている」を選ぶまで出ない）。
- ★**計算をさせない。**単価・数量は明細に出ていれば書き写す欄で、**そこから金額を計算させない・
  金額を上書きしない**（掛け算はこちらでやる）。
- ★**単価・数量のラベルは `<option>` の `data-rate` / `data-qty` が持つ。**
  JS に日本語を置かない＝日英で同じコードが動く。月額固定は `data-rate` が空＝この2欄ごと出さない。
- 払われ方（`method`）は**7択**で日英とも同じ並び ── `monthly` / `duty` / `session` /
...
3b＝超えたときの注意 / 3c＝教官の節を書いた状態 / 3d＝教官を外して消えた状態 / 4＝閉じても残る）と
